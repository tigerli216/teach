...
public class MyResponseBodyAdvice implements ResponseBodyAdvice<Object> {
    ...
    public Object beforeBodyWrite(Object body, MethodParameter returnType, MediaType selectedContentType,
        ...
        try {
            JSONObject jsonObject = Globals.getRequestParamsJson(servletRequest);
            String appSessionId = jsonObject.getString("appSessionId"); //

            String jsonBody = JsonUtils.getJson(body);
            CommonParamsDTO paremDTO = new CommonParamsDTO();
            paremDTO.setAppSessionId(appSessionId);
            UserDTO user = null;
            TutorDTO tutor = null;
            if (StringUtils.isNotBlank(appSessionId) && appSessionId.endsWith("T")) {
                tutor = UserHelper.getTutorDTO(paremDTO);
            } else {
                user = UserHelper.getUserDTO(paremDTO);
            }

            if (user != null || tutor != null) {
                log.debug(new StringBuilder("======APP返回拦截=====用户：")
                        .append(user != null ? user.getLoginAccount() : tutor.getLoginAccount()).append("，Id:")
                        .append(user != null ? user.getUserId() : tutor.getTutorId()).append("，请求URL：").append(url)
                        .append("，请求IP：").append(ip).append("，返回内容：").append(jsonBody).toString());
            } else {
